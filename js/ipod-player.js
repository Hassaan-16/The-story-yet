...
import { AUDIO_PLAYLIST } from './media-manifest.js';
...
(function () {
  ...
  // Load dynamically from media-manifest.js generated by scripts/sync-media.js
  const PLAYLIST = (Array.isArray(AUDIO_PLAYLIST) && AUDIO_PLAYLIST.length > 0)
    ? AUDIO_PLAYLIST
    : FALLBACK_PLAYLIST;
  ...
  // Broadcast iPod state to UI components (e.g. Overview Audio Dock)
  function broadcastState(eventName) {
    if (typeof window !== 'undefined') {
      window.dispatchEvent(new CustomEvent(eventName || 'ipod:statechange', {
        detail: {
  ...
  function nextTrack() {
    ...
    renderScreen();
    broadcastState('ipod:trackchange');
  }
  ...
  // Expose global bridge for external controls (e.g. Overview bottom audio dock)
  window.iPodAudio = {
    togglePlay,
    nextTrack,
    prevTrack,
