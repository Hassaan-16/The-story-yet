 ...
 * e.g. "margalla_sunset.jpg" -> "Margalla Sunset"
 */
...
function syncGallery() {
  ...
  const photos = files.map((file, idx) => {
    const title = formatPhotoTitle(file);
    const encodedName = encodeURIComponent(file);
    return {
      id: `photo-${idx + 1}`,
      title,
...
function main() {
  ...
  // 3. Write js/media-manifest.js (ES Module and global window support for zero-latency loading)
